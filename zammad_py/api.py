...
class ZammadAPI(object):
    ...
    def time_accounting(self):
        """Returns a `TimeAccounting` instance
        """
        return TimeAccounting(connection=self)

    @property
    def ticket_history(self):
        """Returns a `TicketHistory` instance
        """
        return TicketHistory(connection=self)
...
class TimeAccounting(Resource):
    ...
    def by_ticket(self, year, month):
       """Return the resource associated with the id

       :param id: Resource id
       """
       response = self._connection.session.get(
           self.url + '/log/by_ticket/%d/%d' % (year,month)
       )
       return self._raise_or_return_json(response)
